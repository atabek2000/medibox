...
const {
  ...
  () =>
    $fetch(runtimeConfig.public.API_URL + "main/clinics/", {
      headers: {
    ...
    <div class="flex flex-wrap md:flex-nowrap gap-x-[62px]">
      <div class="w-full md:w-1/4 2xl:w-1/5 mt-3 md:mt-0">
        <CommonUiCatalogFilter
          filter_type="sub_organisation_type_id"
      ...
      <div class="w-full md:w-3/4 2xl:w-4/5">
        ...
        >
          <ClinicCard
            v-for="clinic in data?.data?.items"
            :key="clinic.organisation_name"
            :product="clinic"
            class="w-[48%] md:w-fit"
          />
