...
const { data, refresh } = await useAsyncData(() =>
  $fetch(
    useRuntimeConfig().public.API_URL +
      `users/show-item/${route.params.id}?type=${route.query.type}`,
    {
      headers: {
...
<template>
  <div class="rental-page bg-white main-container pb-16">
    <div class="flex justify-between items-center">
      <CommonUiBreadCrumb
        ...
        :links="[
          ...
          { label: $t('bread_crumb.purchase'), to: '/profile/history' },
          { label: $t('bread_crumb.order', { number: data.data?.id }) },
        ]"
      />
    ...
    <div class="flex gap-x-6">
      ...
      <div class="w-full">
        <ProfileOrderHistoryPagePurchase
          :order="data?.data"
          @refresh="refresh"
        />
      </div>
