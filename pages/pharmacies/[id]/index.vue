...
const { data: medicines, refresh: refreshMedicine } = await useAsyncData(
  () =>
    $fetch(
...
const { data: categories } = await useAsyncData(() =>
  $fetch(
    useRuntimeConfig().public.API_URL +
      `entity/medicine/categories/${route.params.id}`
  )
);
    ...
    <div class="pharmacy-page bg-white main-container pb-16">
      <CommonUiBreadCrumb
        ...
        :links="[
          ...
          { label: $t('bread_crumb.pharmacies'), to: '/pharmacies' },
          {
            label:
              data?.data?.[
                locale === 'ru'
          ...
          <div v-if="item.key === 'products'" class="mt-6">
            ...
            <div class="mt-6">
              <nuxt-link
                ...
                :key="product.id"
                :to="`/pharmacies/${$route?.params?.id}/${product.id}`"
              >
