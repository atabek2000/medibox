...
const organization_types = useDicts().value.AreaSale;
const date = ref(new Date(props.user.birthday));

const state = reactive({
  area_sale_id:
    props.user.area_sales && props.user.area_sales.length > 0
      ? props.user.area_sales.map((as) => as.id)
      : [],
...
const profile_edit = async () => {
  for (const phone of state.phones) {
    if (phone.phone.length < 10 || state.phone.length < 10) {
    ...
    >
      <div class="w-full md:w-[48%]">
        ...
        >
          ...
          >
            <template #label>
              <template v-if="state.area_sale_id.length">
                <span class="flex items-center">
                  <span
                    v-for="(label, index) of state.area_sale_id"
                    :key="label"
                  >
                    {{
                      organization_types.filter((so) => so.id === label)[0][
                        `name${locale === "ru" ? "" : "_" + locale}`
                      ]
                    }}{{
                      index === state.area_sale_id.length - 1 ? "" : ",&nbsp;"
                    }}
                    <span></span>
                  </span>
                </span>
