...
const confirmOrder = async () => {
  startLoading();
  await useFetchApi(`entity/accept/medicine-request/${productId.value}`, {
    method: "POST",
...
const setCancelReason = async (id) => {
  ...
  await useFetchApi(`entity/cancel/medicine-request/${productId.value}`, {
    body: {
...
const deleteOrder = async () => {
  startLoading();
  await useFetchApi(`entity/delete/medicine-request/${productId.value}`, {
    method: "DELETE",
    async onResponse({ request, response, options }) {
      stopLoading();
...
onMounted(async () => {
  ...
  if (props.order?.saleMedicines?.[0]?.moderatorPayment?.manifest_id)
    await useFetchApi(
      `delivery/manifest/${props.order?.saleMedicines?.[0]?.moderatorPayment?.manifest_id}`,
      {
        async onResponse({ request, response, options }) {
  ...
  <div class="w-full flex gap-8 flex-wrap md:flex-nowrap">
    <div class="w-full md:w-1/2 2xl:w-1/3">
      <p
        ...
        :class="{
          ' text-indigo':
            order.saleMedicines?.[0]?.delivery?.status === 'pending' ||
            order.saleMedicines?.[0]?.delivery?.status === 'new',
          ' text-valencia':
            order.saleMedicines?.[0]?.delivery?.status === 'canceled',
          ' text-japanese-laurel':
            order.saleMedicines?.[0]?.delivery?.status === 'delivered',
        }"
      >
        {{
          order.saleMedicines?.[0]?.delivery?.status === "canceled" &&
          order.saleMedicines?.[0]?.canceled_user?.id &&
          order.saleMedicines?.[0]?.canceled_user?.id !== cookie_user.id
            ? $t(`fields.order_status.canceled_client`)
            : $t(
                `fields.order_status.${order.saleMedicines?.[0]?.delivery?.status}`
              )
        }}
      </p>
      <div class="flex gap-4 mt-6">
        <img
          ...
          :src="
            order.saleMedicines?.[0]?.moderatorPayment?.user?.avatar[0]
              ? order.saleMedicines?.[0]?.moderatorPayment?.user?.avatar[0]
              : '/common/avatar.png'
          "
          alt="avatar"
        ...
        <div>
          <p class="text-base font-bold">
            {{
              order.saleMedicines?.[0]?.moderatorPayment?.user?.first_name
                ? `${order.saleMedicines?.[0]?.moderatorPayment?.user?.first_name} ${order.saleMedicines?.[0]?.moderatorPayment?.user?.last_name}`
                : order.saleMedicines?.[0]?.moderatorPayment?.user
                    ?.contact_person
            }}
          </p>
          <div class="flex gap-1 mt-2">
            ...
            <span class="font-normal text-sm">{{
              changePhoneMask(
                "7" + order.saleMedicines?.[0]?.moderatorPayment?.user?.phone
              )
            }}</span>
          </div>
          <div class="flex gap-1 mt-2">
            ...
            <span class="font-normal text-sm">
              {{
                dateToString(
                  order.saleMedicines?.[0]?.moderatorPayment?.created_at.split(
                    "T"
                  )[0]
                )
              }},
              {{
                new Date(order.saleMedicines?.[0]?.moderatorPayment?.created_at)
      ...
      <div class="mt-4">
        ...
        <!-- <div class="flex justify-between">
          ...
          <p class="text-base font-medium">
            {{ priceFormat(order.moderatorPayment?.price) }}
          </p>
        </div> -->
      </div>

      <div
        v-for="product in order.saleMedicines"
        :key="product.id"
        class="mb-6"
      >
        <div class="mt-6 mb-4">
          ...
          <p class="text-base font-normal text-heavy-metal-200">
            {{
              product.delivery?.delivery_type === "pickup"
                ? $t("basket.self_pick_up") +
                  " " +
                  $t("content.to_date", {
                    date: dateToString(product.delivery?.date),
                  })
                : dateToString(product.delivery?.date)
            }}
          </p>
          <p class="text-base font-normal text-heavy-metal-200">
            {{
              product.delivery?.delivery_type === "pickup"
                ? product.userProduct.user.address
                : product.moderatorPayment.user.address
            }}
          </p>
        </div>
        <nuxt-link
          :to="`/pharmacies/${product?.userProduct?.user?.id}/${product?.userProduct?.id}`"
          class="border border-wild-sand-600 rounded-xl p-4 flex gap-3"
        >
          <img
            class="w-24 h-20 object-cover"
            :src="
              product?.userProduct?.medicine?.photo[0] ?? '/common/avatar.png'
            "
            alt="product"
          />
          <div>
            ...
            <p class="mt-3 text-base font-medium">
              {{ priceFormat(product?.userProduct?.price) }}
            </p>
          </div>
        </nuxt-link>
        ...
        <UButton
          ...
          @click="setConfirmOrderVisible(true, product.id)"
          >{{ $t("button.confirm") }}</UButton
        >
        <UButton
          v-if="product?.status === 'new' || product?.status === 'pending'"
          class="w-full bg-valencia-200 hover:bg-valencia-100 text-valencia mt-3"
          color="valencia"
          :ui="TransparentButtonConfig()"
          @click="setCancelOrderVisible(true, product.id)"
        ...
        <UButton
          ...
          color="valencia"
          :ui="TransparentButtonConfig()"
          @click="setDeleteOrderVisible(true, product.id)"
    ...
    <div
      ...
      v-if="
        manifest_url !== '' &&
        order?.saleMedicines?.[0]?.moderatorPayment?.manifest_id
      "
    >
