...
const selectedAvatar = ref(["/common/avatar.png"]);
...
const onAddSpec = () => {
  ...
  startLoading();
  const spec = useFetchApi("services/create-specialist", {
    method: "POST",
    body: form_data,
    headers: {
    ...
    async onResponse({ request, response, options }) {
      stopLoading();
      if (response.ok) useRouter().push("/profile/specialist");
    },
  });
};
