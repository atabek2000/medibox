...
const isVisibleSpecialists = ref(false);
...
const deleteService = async () => {
  startLoading();
  const res = await useFetchApi(`services/delete-service/${route.params.id}`, {
    method: "DELETE",
    onResponse() {
      stopLoading();
    },
...
const save = async () => {
      ...
      for (let i = 1; i <= 7; i++) {
        ...
        if (ss[`d_${i}`].is_weekend) continue;
          ...
          useToast().add({
            color: "red",
            title: t("toast.error"),
            description: t(`toast.set_spec_work_time`, { name: `${ss.first_name} ${ss.last_name}` }),
          });
          return;
        }
  ...
  startLoading();
  const res = await useFetchApi(`services/edit-service/${route.params.id}`, {
    method: "POST",
    body: form_data,
    headers: {
...
onBeforeMount(() => {
  selected_specialists.value = props.service.employees.map((emp) => {
    const spec_days = {};
    for (const eday of emp.days) {
      spec_days[eday.work_day] = {
        is_weekend: !!eday.is_weekend,
        time: {
          start: {
            hour: splitDBTime(eday.start_work_time).hour,
            minute: splitDBTime(eday.start_work_time).minute,
          },
          end: {
            hour: splitDBTime(eday.end_work_time).hour,
            minute: splitDBTime(eday.end_work_time).minute,
          },
        },
      };
    }
    ...
    return {
      id: emp.specialist.id,
      last_name: emp.specialist?.last_name,
      first_name: emp.specialist?.first_name,
      ...spec_days,
    };
  });
...
<template>
  <div class="add-service">
    <UForm
      ...
      @submit="save"
    >
      <div class="w-full flex flex-wrap md:flex-nowrap gap-3 md:gap-6">
