...
const state = reactive({
  ...
  placing: props.room.placing,
  condition: props.room.condition,
  duration: props.room.duration,
  phone: changePhoneMask("7" + props.room.phone),
  is_acquainted: false,
  ceiling_height: Number(props.room.ceiling_height),
  room_count: Number(props.room.room_count),
  square: Number(props.room.square),
  year_construction: Number(props.room.year_construction),
  price: parseInt(props.room.price).toString(),
  purpose_id: Number(props.room.purpose?.id),
  city_id: Number(props.room.city?.id),
});
const phoneModel = ref(changePhoneMask("7" + props.room.phone));
const cities = ref([]);
const purposes = ref([]);
const durations = ref([]);
...
const onAddPremis = () => {
  ...
  startLoading();
  const spec = useFetchApi(`premises/edit-premise/${route.params.id}`, {
    method: "POST",
    body: form_data,
    headers: {
...
const deleteProduct = async () => {
  startLoading();
  const res = await useFetchApi(`premises/delete-premise/${route.params.id}`, {
    method: "DELETE",
    onResponse() {
      stopLoading();
    },
  ...
  <div>
    ...
    >
      ...
      <div class="w-full md:w-[48%]">
        ...
        >
          <div class="images-block flex gap-x-2">
            <img
              v-for="(image, index) in room.photo"
              :src="image"
              :key="index"
              alt="room"
      ...
      <UButton
        ...
        >{{ $t("button.delete_room") }}</UButton
      >
    </UForm>
    <CommonUiModalConfirm
      v-model="logoutConfirmVisible"
      :title="$t('modal.delete')"
      :body="$t('modal.delete_room')"
